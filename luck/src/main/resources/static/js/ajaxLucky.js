$(function () {
    ...
    Obj.luckyPrizeTitle = $(".lucky_prize_title").text();
    /*
     一次抽几人改变事件
     */
    ...
    function loadImage(arr, callback) {
     ...
     //若为ajax请求执行这段代码
     //此为为ajax请求;
     $.get('/info/meeting/user/',function(data){
         if(data.code == 0){
             personArray = data.obj; //此为数组
             var len = data.obj.length;
             $(".lucky_number").text(len);
    ...
    /*
     ...
     传入当前中奖数组中单个的key
     */
    function showLuckyPeople(people,num) {
        setTimeout(function () {
            ...
            $('.mask').append($fragEle);
            $(".mask").fadeIn(200);
            $luckyEle.attr('src', '/static/img/'+people.name+".png");
    ...
    $.get('/info/meeting/all/prize/',function(data){
        if(data.code == 0){
            var prizeList = data.obj;
            var prizeText = '';
            if (prizeList){
                for (var i = 0; i < prizeList.length; i++) {
                    var prize = prizeList[i];
                    prizeText += ' <img class="lucky_prize_show none"  src="' + prize.image +'"  alt="' + prize.title + '"/> ';
                }
            }
            $(".lucky_prize_picture").html(prizeText);
            tabPrize();
        }
    },'json')
    ...
    $('#stop').click(function () {
        ...
        for (; i < Obj.luckyResult.length; i++) {
            showLuckyPeople(Obj.luckyResult[i],i);
        }
    ...
    $('#open').click(function () {
        ...
        Obj.M.open();
        //此为ajax请求获奖结果
        $.post('/info/meeting/prize/',{"lucky_num" : Obj.luckyNum,"lucky_prize":Obj.luckyPrizeTitle},function(data){
        	  if(data.code == 0){
        		  Obj.luckyResult = data.obj;
               $("#stop").show(500);
        	  }
        },'json')
        //ajax获奖结果结束
    ...
    function tabSport(i, lucky_prize_number) {
        ...
        Obj.luckyPrizeTitle = $('.lucky_prize_show').eq(i - 1).attr('alt');
